							...
							<li>
								...
								<div id="dv1">
									<table class="table" id="tbRecord">
										...
										<tbody>
                                        <c:forEach var="outProduct" items="${OutProducts}">
                                            <tr>
                                                <td>${outProduct.opid }</td>
                                                <td>${outProduct.pname }</td>
                                                <td>${outProduct.cname }</td>
                                                <td>${outProduct.onu }</td>
                                                <td>${outProduct.odate }</td>
                                                <td>${outProduct.uname }</td>
                                                <td>${outProduct.ointrog }</td>
                                                <td class="delete"><button onclick="btn_delete(${outProduct.opid })"><i class="icon-trash bigger-120"></i>删除</button></td>
                                            </tr>
                                        </c:forEach>
										</tbody>
						...
						<form class="am-form am-form-horizontal"
							...
							<div class="am-form-group">
								<label for="user-intro" class="am-u-sm-3 am-form-label">
									备注</label>
								<div class="am-u-sm-9">
									<textarea class="" rows="5" id="user-intro" name="ointrog"
			...
			<script>
				...
				var btn_delete = function(id) {
					$.jq_Confirm({
						message: "您确定要删除吗?",
						btnOkClick: function() {
							location.href="${pageContext.request.contextPath }/OutProduct/delete/"+id;
						}
					});
				}
